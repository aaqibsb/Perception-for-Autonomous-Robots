...
def estimate_disparity_depth():
    ...
    for y1 in np.arange(3, Height-3):
        for x1 in np.arange(3, Width-3):
            ...
            if int(disparity) == 0:
                disparity = 0.00001
                depth[x1][y1] = (1758.23 * 97.99)/disparity
            else:
                depth[x1][y1] = (1758.23 * 97.99)/disparity
...
if __name__ == '__main__':
    # Reading Images
    data_img1 = cv.imread("depth_files/chess/im0.png")
    data_img2 = cv.imread("depth_files/chess/im1.png")
    ...
    F = ransac(matched_points, 0.1, 20)
    print("Fundamental Matrix:\n",F,sep='')
    print()

    K_intrinsic = np.array([[1758.23, 0, 829.15],
                            [0, 1758.23, 552.78],
                            [0,       0,      1]])
