...
def estimate_disparity_depth():
    ...
    for y1 in np.arange(3, Height-3):
        for x1 in np.arange(3, Width-3):
            ...
            if int(disparity) == 0:
                disparity = 0.00001
                depth[x1][y1] = (1734.16 * 228.38)/disparity
            else:
                depth[x1][y1] = (1734.16 * 228.38)/disparity
...
if __name__ == '__main__':
    # Reading Images
    data_img1 = cv.imread("depth_files/ladder/im0.png")
    data_img2 = cv.imread("depth_files/ladder/im1.png")
    ...
    F = ransac(matched_points, 0.1, 30)
    print("Fundamental Matrix:\n",F,sep='')
    print()

    K_intrinsic = np.array([[1734.16, 0, 333.49],
                            [0, 1734.16, 958.05],
                            [0,       0,      1]])
